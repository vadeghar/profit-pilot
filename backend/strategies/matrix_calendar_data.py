...
def get_expiry_pair(entry_date: date) -> tuple[date, date] | None:
    """Return the next usable weekly expiry and following monthly expiry."""
    query = text(
        """
    ...
    weekly = next((r["expiry_date"] for r in rows if r["expiry_type"] == "WEEKLY"), None)
    monthly = next(
        (
            r["expiry_date"]
            for r in rows
            if r["expiry_type"] == "MONTHLY" and r["expiry_date"] > (weekly or entry_date)
        ),
        None,
    )
