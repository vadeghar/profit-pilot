...
import { analyticsInitial, type AnalyticsSnapshot } from '../types/analytics';
import { streamAnalytics } from '../services/analyticsApi';
...
export function AnalyticsView(){
 const [data,setData]=useState<AnalyticsSnapshot>(analyticsInitial);
 ...
 const loadAnalytics=(forceRefresh=false)=>{
  streamRef.current?.();
  setLoading(true); setError(null);
 streamRef.current=streamAnalytics(
   (card,value,meta)=>setData(current=>({...current,[card]:value,meta})),
   snapshot=>{setData(snapshot);setLoading(false);},
   ()=>{setLoading(false);setError('Analytics stream unavailable; showing the latest available values.');},
   forceRefresh,
   ()=>setLoading(true),
  );
 };
 useEffect(()=>{const timer=window.setTimeout(()=>loadAnalytics(),0); return()=>{window.clearTimeout(timer);streamRef.current?.();};},[]);
 const value=(number:number|null, suffix='')=>number===null?'—':`${suffix==='%'||!Number.isInteger(number)?number.toFixed(2):number}${suffix}`;
 const stats:[string,string][]=[['Advancers',value(data.summary.advancers)],['Decliners',value(data.summary.decliners)],['New Highs',value(data.summary.newHighs)],['New Lows',value(data.summary.newLows)]];
 const adv=data.breadth.advancingPercent??0;
 const leaders=data.momentumLeaders;
 const alert=error||data.meta.errors[0];
 const refreshButton=<button onClick={()=>loadAnalytics(true)} disabled={loading} className="flex h-8 items-center gap-2 border border-[#30363D] bg-[#161B22] px-3 text-xs text-[#8B949E] hover:text-white disabled:cursor-not-allowed disabled:opacity-60"><RefreshCw className={`h-3.5 w-3.5 ${loading?'animate-spin':''}`}/>Refresh</button>;
 return <DataWorkspace title="Analytics" subtitle="Market breadth, momentum and performance analytics" stats={stats} actions={<div className="flex gap-2">{refreshButton}</div>}>
  {alert&&<div className="border border-[#D29922] px-3 py-2 text-[10px] text-[#D29922]">{alert}</div>}
  <div className="grid gap-3 lg:grid-cols-3"><Panel title="Market Breadth"><div className="p-4"><div className="num text-3xl">{loading&&data.summary.advanceDeclineRatio===null?'…':value(data.summary.advanceDeclineRatio)}</div><div className="mt-1 text-xs text-[#8B949E]">Advance / decline ratio</div><div className="mt-5 h-3 bg-[#F85149]"><div className="h-full bg-[#2EA043] transition-all" style={{width:`${adv}%`}}/></div><div className="mt-2 flex justify-between text-[10px] text-[#8B949E]"><span>{value(data.summary.advancers,' Adv')}</span><span>{value(data.summary.decliners,' Dec')}</span></div></div></Panel><Panel title="Momentum Leaders"><div className="divide-y divide-[#21262D]">{loading&&!leaders.length&&[1,2,3,4,5].map(i=><div key={i} className="flex items-center gap-2 p-3"><span className="h-3 w-5 animate-pulse bg-[#21262D]"/><span className="h-3 flex-1 animate-pulse bg-[#21262D]"/><span className="h-3 w-12 animate-pulse bg-[#21262D]"/></div>)}{leaders.map((leader,i)=><div key={leader.symbol} className="flex items-center p-3"><span className="w-5 num text-[#8B949E]">{i+1}</span><span className="flex-1">{leader.symbol}</span><span className={`num ${leader.changePct!==null&&leader.changePct>=0?'text-[#2EA043]':'text-[#F85149]'}`}>{value(leader.changePct,'%')}</span><MiniSpark positive={leader.changePct===null||leader.changePct>=0}/></div>)}{!loading&&!leaders.length&&<div className="p-4 text-xs text-[#8B949E]">No recent market data.</div>}</div></Panel><Panel title="Volatility"><div className="space-y-4 p-4">{loading&&!data.volatility.length&&[1,2,3,4].map(i=><div key={i} className="flex justify-between border-b border-[#21262D] pb-3"><span className="h-3 w-20 animate-pulse bg-[#21262D]"/><span className="h-3 w-16 animate-pulse bg-[#21262D]"/></div>)}{data.volatility.map(metric=><div key={metric.name} className="flex justify-between border-b border-[#21262D] pb-3"><span className="text-xs text-[#8B949E]">{metric.name}</span><span className="num">{value(metric.value,metric.unit)} <span className="ml-2 text-[#8B949E]">{value(metric.changePct,'%')}</span></span></div>)}{!loading&&!data.volatility.length&&<div className="text-xs text-[#8B949E]">No recent volatility data.</div>}</div></Panel></div>
<div className="grid gap-3 lg:grid-cols-2"><Panel title="NSE Market Movers"><div className="grid grid-cols-1 divide-y divide-[#21262D] sm:grid-cols-2 sm:divide-x sm:divide-y-0"><div className="min-w-0 p-3"><div className="mb-2 text-[10px] text-[#8B949E]">TOP GAINERS</div>{(data.marketCards.gainers||[]).map(row=><div key={row.symbol} className="flex min-w-0 items-center justify-between gap-2 border-b border-[#21262D] py-1.5 text-[11px]"><span className="min-w-0 truncate" title={row.name||row.symbol}>{row.name||row.symbol}</span><span className="num shrink-0 text-[#2EA043]">{value(row.changePct,'%')}</span></div>)}</div><div className="min-w-0 p-3"><div className="mb-2 text-[10px] text-[#8B949E]">TOP LOSERS</div>{(data.marketCards.losers||[]).map(row=><div key={row.symbol} className="flex min-w-0 items-center justify-between gap-2 border-b border-[#21262D] py-1.5 text-[11px]"><span className="min-w-0 truncate" title={row.name||row.symbol}>{row.name||row.symbol}</span><span className="num shrink-0 text-[#F85149]">{value(row.changePct,'%')}</span></div>)}</div></div></Panel><Panel title="Most Active"><div className="grid grid-cols-1 divide-y divide-[#21262D] sm:grid-cols-2 sm:divide-x sm:divide-y-0"><div className="min-w-0 p-3"><div className="mb-2 text-[10px] text-[#8B949E]">BY VALUE</div>{(data.marketCards.activeValue||[]).map(row=><div key={row.symbol} className="flex min-w-0 items-center justify-between gap-2 border-b border-[#21262D] py-1.5 text-[11px]"><span className="min-w-0 truncate" title={row.name||row.symbol}>{row.name||row.symbol}</span><span className="num shrink-0">{value(row.price)}</span></div>)}</div><div className="min-w-0 p-3"><div className="mb-2 text-[10px] text-[#8B949E]">BY VOLUME</div>{(data.marketCards.activeVolume||[]).map(row=><div key={row.symbol} className="flex min-w-0 items-center justify-between gap-2 border-b border-[#21262D] py-1.5 text-[11px]"><span className="min-w-0 truncate" title={row.name||row.symbol}>{row.name||row.symbol}</span><span className="num shrink-0">{row.volume?.toLocaleString('en-IN')||'—'}</span></div>)}</div></div></Panel><Panel title="52-Week Activity"><div className="grid grid-cols-1 divide-y divide-[#21262D] sm:grid-cols-2 sm:divide-x sm:divide-y-0"><div className="min-w-0 p-3"><div className="mb-2 text-[10px] text-[#8B949E]">NEW HIGHS</div>{(data.marketCards.newHighs||[]).map(row=><div key={row.symbol} className="flex min-w-0 items-center justify-between gap-2 border-b border-[#21262D] py-1.5 text-[11px]"><span className="min-w-0 truncate" title={row.name||row.symbol}>{row.name||row.symbol}</span><span className="num shrink-0 text-[#2EA043]">{row.price===null?'Count':value(row.price)}</span></div>)}</div><div className="min-w-0 p-3"><div className="mb-2 text-[10px] text-[#8B949E]">NEW LOWS</div>{(data.marketCards.newLows||[]).map(row=><div key={row.symbol} className="flex min-w-0 items-center justify-between gap-2 border-b border-[#21262D] py-1.5 text-[11px]"><span className="min-w-0 truncate" title={row.name||row.symbol}>{row.name||row.symbol}</span><span className="num shrink-0 text-[#F85149]">{value(row.price)}</span></div>)}</div></div></Panel><Panel title="Most Active Contracts"><div className="grid grid-cols-1 divide-y divide-[#21262D] sm:grid-cols-3 sm:divide-x sm:divide-y-0">{([['mostActiveCall','MOST ACTIVE CALLS'],['mostActivePut','MOST ACTIVE PUTS'],['mostActiveContractbyOI','MOST ACTIVE CONTRACTS BY OI']] as const).map(([key,label])=><div key={key} className="min-w-0 p-3"><div className="mb-2 text-[10px] text-[#8B949E]">{label}</div>{(data.activeContracts[key]||[]).slice(0,5).map(row=><div key={row.contract} className="border-b border-[#21262D] py-1.5 text-[11px]"><div className="truncate" title={row.contract}>{row.contract}</div><div className="mt-0.5 flex justify-between gap-2 text-[10px]"><span className="num">{value(row.price)}</span><span className={row.changePct!==null&&row.changePct>=0?'num text-[#2EA043]':'num text-[#F85149]'}>{value(row.changePct,'%')}</span></div></div>)}</div>)}</div></Panel></div>
  <div className="flex justify-between text-[10px] text-[#8B949E]"><span>Source: {data.meta.source||'Waiting for market data'} · {data.meta.freshness}</span><span>{loading?'Updating analytics...':data.meta.asOf?`Updated ${new Date(data.meta.asOf).toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit',hour12:false})}`:'Not available'}</span></div>
 </DataWorkspace>
}
